               ...
               Student.findOne({where:{index:a.index}}).then(function(student){
                ...
                Student.findOne({where:{index:a.index}}).then(function(student){
                    if (student == null) {
                       ...
                       res.status(200).send("Postoji lik");
                    }
                })
...
app.put('/student/:index',(req, res) => {
    ...
    a = JSON.parse(JSON.stringify(a));
    console.log(a.grupa);
    let indeksKojiJePoslan = req.url.substr(10);
    ...
    Student.findOne({where:{index:indeksKojiJePoslan}}).then(function(student) {
        if (student == null) {
            ...
            res.status(200).send("Student ne postoji");
        }
        else {
            Grupa.findOne({where:{naziv:a.grupa}}).then(function(grupa) {
                if (grupa == null) {
...
function pokusaj(red1) {
    ...
    return new Promise(function(resolve,rejects) {
        setTimeout(function () {
          nadjiGrupu(red1[3]).then(prva,druga);
...
app.post('/vjezbe',(req, res) => {
    ...
    Student.findAll({attributes: ['id'],raw:true}).then(function(studenti)  {
